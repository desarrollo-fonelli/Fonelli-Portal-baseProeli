...
import { ServicioCarriers } from 'src/app/services/carriers.service';
...
export class GuiasComponent implements OnInit, OnDestroy {
  ...
  oBuscar: FiltrosGuias;
  oGuiaRes: ConsultaGuias;
  public oBuscaDetalle: FiltrosDetalleGuia;
  oPedidoDetalleRes: DetalleGuia;
  ...
  constructor(
    ...
    private _servicioCarriers: ServicioCarriers,
    private _servicioConGuias: ServicioConsultaGuias,
    private _servicioConGuiasDet: ServicioConsultaGuiasDet
  ) {
  ...
  consultaDetalleGuia(folio: String) {
    ...
    this._servicioConGuiasDet.Get(this.oBuscaDetalle).subscribe(
      (Response: DetalleGuia) => {
        ...
        if (this.oPedidoDetalleRes.Codigo != 0) {
          this.bError = true;
          this.sMensaje = 'No se encontro detalle de guia';
