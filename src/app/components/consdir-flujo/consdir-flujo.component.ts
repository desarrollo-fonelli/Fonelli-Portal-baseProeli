...
import { FlujoResponse, TiposFlujo, Flujo } from './modelos/consdir-flujo.model';

// Servicios
import { ConsdirFlujoService } from './servicios/consdir-flujo.service';
...
@Component({
  selector: 'app-consdir-flujo',
  templateUrl: './consdir-flujo.component.html',
  styleUrls: ['./consdir-flujo.component.css'],
  providers: [DecimalPipe, FuncFechasService, FuncStringsService,
    ConsdirFlujoService]
})
export class ConsdirFlujoComponent implements OnInit {
  ...
  oBuscar: FlujoFiltros = {
    TipoUsuario: '', Usuario: 0, FechaDesde: '', FechaHasta: '', Pagina: 1
  };

  oFlujoResponse: FlujoResponse;
  oFlujosContenido: Flujo[];
  ...
  constructor(
    ...
    private _consdirFlujoService: ConsdirFlujoService
  ) {
    ...
    this.oFlujoResponse = {} as FlujoResponse;
    this.oFlujosContenido = [] as Flujo[];
  }
  ...
  */
  ConsultaFlujoEfectivo() {
    ...
    this._consdirFlujoService.GetConsdirFlujo(this.oBuscar)
      .pipe(finalize(() => { this.bCargando = false }))
      .subscribe({
        next: (response: FlujoResponse) => {

          this.oFlujoResponse = response;

          if (this.oFlujoResponse.Codigo == 0) {
            this.bError = false;
            this.sMensaje = '';
            this.isCollapsed = true;

            this.oFlujosContenido = this.oFlujoResponse.Contenido?.FlujosContenido || [];
            console.dir(this.oFlujosContenido);
            //this.CalcularTotales();   todavia no existe esta funcion
            ...
            this.bError = true;
            this.isCollapsed = false;
            this.sMensaje = this.oFlujoResponse.Mensaje || 'No se obtuvieron datos para los filtros seleccionados';
          }
        },
        error: (err) => {
