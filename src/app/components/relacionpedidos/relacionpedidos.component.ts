...
import {
  ...
  Pedido,
  TipoPedido,
  Contenido as ContenidoGen,
} from 'src/app/models/relacionpedidos';
import { FiltrosOficina } from 'src/app/models/oficina.filtros';
import { Oficina } from 'src/app/models/oficina';
import { FiltrosDetallePedidos } from 'src/app/models/detallepedido.filtros';
import { DetallePedido, PedidoArticulo } from 'src/app/models/detallepedido';
>>>>>>> 96dca9a1f4fe4becea9180c2016da41ba36df688
...
>>>>>>> 96dca9a1f4fe4becea9180c2016da41ba36df688
    ...
    this.oBuscar = new FiltrosRelacionPedidos(
      '',
      0,
      '',
      '',
  ...
  ngOnInit(): void {
    ...
    switch (this.sTipo) {
      case 'C': {
        ...
        this._servicioOficinas.Get(this.oBuscarOfi).subscribe(
          (Response: Oficina) => {
            this.oOficinasRes = Response;
            //console.log("RESULTADO LLAMADA Oficinas "+JSON.stringify(this.oOficinasRes) );
            ...
            this.oBuscar.OficinaDesde =
              this.oOficinasRes.Contenido[0].OficinaCodigo;
            this.oBuscar.OficinaHasta =
              this.oOficinasRes.Contenido[
                this.oOficinasRes.Contenido?.length - 1
              ].OficinaCodigo;
            this.sMensaje = '';
          },
          (error: Oficina) => {
...
>>>>>>> 96dca9a1f4fe4becea9180c2016da41ba36df688
  ...
  getTotalCanPed(Pedido: Pedido[]): number {
    let total = Pedido.map((item) => item.CantidadPedida).reduce(
  ...
  getTotalImportes(Pedido: Pedido[]): number {
    let total = Pedido.map((item) => item.CantidadPedidaImporte).reduce(
      (total, actual) => total + actual,
      0
    );
    total = Number(total.toFixed(2));
  ...
  getTotalCPValorAgregado(Pedido: Pedido[]): number {
    let total = Pedido.map((item) => item.CantidadPedidaValorAgregado).reduce(
      (total, actual) => total + actual,
      0
    );
    total = Number(total.toFixed(2));
  ...
  getTotalCanSurtida(Pedido: Pedido[]): number {
    let total = Pedido.map((item) => item.CantidadSurtida).reduce(
      (total, actual) => total + actual,
      0
    );
    total = Number(total.toFixed(2));
...
=======
  getTotalCanSurtidaImporte(Pedido: Pedido[]): number {
    let total = Pedido.map((item) => item.CantidadSurtidaImporte).reduce(
      (total, actual) => total + actual,
      0
    );
    total = Number(total.toFixed(2));
...
>>>>>>> 96dca9a1f4fe4becea9180c2016da41ba36df688
  ...
  getTotalCSValorImporte(Pedido: Pedido[]): number {
    let total = Pedido.map((item) => item.CantidadSurtidaValorAgregado).reduce(
      (total, actual) => total + actual,
      0
    );
    total = Number(total.toFixed(2));
  ...
  getTotalDifCantidadPro(Pedido: Pedido[]): number {
    let total = Pedido.map((item) => item.DiferenciaCantidadProducido).reduce(
      (total, actual) => total + actual,
      0
    );
    total = Number(total.toFixed(2));
  ...
  getTotalDifImporteSurtido(Pedido: Pedido[]): number {
    let total = Pedido.map((item) => item.DiferenciaImporteSurtido).reduce(
      (total, actual) => total + actual,
      0
    );
    total = Number(total.toFixed(2));
  ...
  getTotalDifValorAgregado(Pedido: Pedido[]): number {
    let total = Pedido.map((item) => item.DiferenciaValorAgregado).reduce(
      (total, actual) => total + actual,
      0
    );
    total = Number(total.toFixed(2));
  ...
  getTotalCantPedidaProd(Pedido: Pedido[]): number {
    let total = Pedido.map((item) => item.CantidadPedidaProduccion).reduce(
      (total, actual) => total + actual,
      0
    );
    total = Number(total.toFixed(2));
  ...
  getTotalCantidadProd(Pedido: Pedido[]): number {
    let total = Pedido.map((item) => item.CantidadProducida).reduce(
      (total, actual) => total + actual,
      0
    );
    total = Number(total.toFixed(2));
  ...
  //##### TOTALES OFICINA #####
  getTotalPedidosOficina(TipoPedido: TipoPedido[]): number {
    let total = TipoPedido.map((item) => item.Pedidos.length).reduce(
      (total, actual) => total + actual,
      0
    );
    return total;
